...
public class CustomNavigationView extends BottomNavigationView {
    ...
    private void initAttrs(Context context, AttributeSet attrs, int defStyleAttr) {
        curveCircleRadius = context.getResources().getDimensionPixelSize(R.dimen.nbavex_curve_radius_default);
        if(attrs != null) {
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mFirstCurveControlPoint1.set(mFirstCurveStartPoint.x + curveCircleRadius + (curveCircleRadius / 4), mFirstCurveStartPoint.y);
        // the coordinates (x,y)  of the 2nd control point on a cubic curve
        mFirstCurveControlPoint2.set(mFirstCurveEndPoint.x - (curveCircleRadius * 2) + curveCircleRadius, mFirstCurveEndPoint.y);

        mSecondCurveControlPoint1.set(mSecondCurveStartPoint.x + (curveCircleRadius * 2) - curveCircleRadius, mSecondCurveStartPoint.y);
        mSecondCurveControlPoint2.set(mSecondCurveEndPoint.x - (curveCircleRadius + (curveCircleRadius / 4)), mSecondCurveEndPoint.y);
